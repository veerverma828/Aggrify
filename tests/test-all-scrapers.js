...
async function testAll() {
  try {
    ...
    const locationInfo = LOCATIONS['bengaluru'];

    console.log('\n--- Testing Blinkit ---');
    try {
      await scrapeBlinkit(query, locationInfo, onProducts, isCancelled);
    } catch (e) {
    ...
    console.log('\n--- Testing Zepto ---');
    try {
      await scrapeZepto(query, locationInfo, onProducts, isCancelled);
    } catch (e) {
